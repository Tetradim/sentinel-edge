...
def test_set_stop_serializes_absolute_tighten_only_policy():
    command = HandoffCommand(
        symbol="SPY",
...
async def test_emitter_sends_reduce_as_sell_envelope_with_typed_directive():
    ...
    class _Scheduler:
        ...
        async def _handoff_to_pulse_with_feedback(self, **kwargs):
            calls.append(kwargs)
            return {"sent": True, "accepted": True, "status": "accepted", "reason": "pulse_accepted"}
    ...
    token = brain_runtime._BRAIN_CONTEXT.set(context)
    try:
        await supervision._emit_supervisory_directive(
            scheduler,
            "SPY",
            analysis,
...
def test_production_install_keeps_enhanced_wrapper_under_supervision():
    import edge_brain_patch  # noqa: F401

    assert DecisionEngine.decide is supervision._decide_with_supervision
