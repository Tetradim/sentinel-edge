...
def _emergency_active(engine: DecisionEngine, symbol: str, kwargs: Dict[str, Any]) -> bool:
    max_losses = kwargs.get("max_consecutive_losses")
    max_losses = int(max_losses if max_losses is not None else engine.MAX_CONSECUTIVE_LOSSES)
    max_drawdown = kwargs.get("max_drawdown_pct")
    max_drawdown = float(max_drawdown if max_drawdown is not None else engine.MAX_DRAWDOWN_PCT)
    return bool(
...
def _set_context(
    ...
    *,
    ...
    # edge_brain_runtime's post-evaluation hook is keyed by this legacy value;
    # the typed directive selects the concrete emitted action.
...
def _decide_with_supervision(self: DecisionEngine, *args, **kwargs) -> Decision:
    ...
    if signal <= -sell_threshold and confidence >= sell_confidence:
        _set_context(
            context,
            directive="sell",
            reason=f"Severe enhanced bearish thesis invalidated the position (signal={signal:.2f}, confidence={confidence:.2f})",
    ...
    if signal <= -reduce_threshold and confidence >= reduce_confidence:
        ...
        _set_context(
            ...
            reason=f"Confirmed bearish deterioration requires exposure reduction (signal={signal:.2f}, confidence={confidence:.2f})",
            expected_quantity=expected_quantity,
    ...
    if signal <= -stop_threshold and confidence >= stop_confidence:
        ...
        if stop_price is not None:
            _set_context(
                context,
                directive="set_stop",
                reason=f"Early bearish deterioration requires a concrete protective stop (signal={signal:.2f}, confidence={confidence:.2f})",
                expected_quantity=expected_quantity,
...
def _record_feedback(scheduler: Any, symbol: str, analysis: AnalysisResult, directive: str, feedback: Any) -> None:
    ...
    if ticker_state is not None:
        ticker_state.update({"last_decision": directive, "supervisory_override": True, "pulse_feedback": feedback})
...
async def _emit_supervisory_directive(
    ...
    decision_id = f"edge-supervision:{symbol.upper()}:{directive}:{round(float(value or 0.0), 4)}:{int(time.time() // 30)}"
    metadata = {
        ...
        "max_quantity_drift_percent": env_float("EDGE_BRAIN_POSITION_GUARD_DRIFT_PERCENT", 2.0, minimum=0.0),
        "decision_id": decision_id,
        "brain_version": (analysis.metadata or {}).get("brain_version", "edge-brain-v1"),
    }
    if directive == "set_stop":
        metadata.update({"stop_price": context.get("stop_price"), "tighten_only": True})
    ...
    _record_feedback(scheduler, symbol, analysis, directive, feedback)
