...
class ProfitabilityLifecycleMixin:
    ...
    def observe_position(self, symbol: str, position: Optional[Dict[str, Any]], current_price: float = 0.0) -> None:
        ...
        entry = finite(position.get("entry_price", position.get("avg_entry", position.get("average_price", 0.0))))
        pnl = finite(position.get("current_pnl_dollar", position.get("pnl", position.get("realized_pnl", 0.0))))
        pnl_pct = finite(position.get("current_pnl_pct", position.get("pnl_pct", position.get("realized_return_pct", 0.0))))
